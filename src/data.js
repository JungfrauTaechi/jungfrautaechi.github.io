...
export const utilityLinks = [{ label: "Wind, Meteo & Webcams", to: routes.meteo.path }, { label: "Sicherheit & Regeln", to: routes.safety.path, className: "safety-link" }, { label: "DABS", href: "https://www.skybriefing.com/de/" }];
...
export const flightFacts = [{ title: "9 Panorama-Standorte", body: "4 Übersichten · 3 Startplätze · 2 Landeplätze", path: routes.flightArea.path }, { title: "360° vor Ort", body: "Start- und Landeplätze vor dem Flug erkunden", path: routes.landingSites.path }, { title: "Sicherheit", body: "Lokale Regeln, Lufträume und DABS vor jedem Flug", path: routes.safety.path }];
...
const publicSceneIds = new Set([...overviewSites, ...startSites, ...landingSites].map((site) => site.id));
const withSceneMeta = (site, sceneType) => ({ ...site, sceneType, links: (panoramaLinks[site.id] || []).filter((link) => publicSceneIds.has(link.targetId)), landmarks: panoramaLandmarks[site.id] || [], areas: panoramaAreas[site.id] || [] });
