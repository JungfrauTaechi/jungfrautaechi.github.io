...
function LocalPanorama({ site, reloadKey, onSelectScene, onOpenMeteo, showAreas }) {
  ...
  return <div className={`panorama-layer${selectingPosition ? " is-picking" : ""}`} onClickCapture={pickPosition}><div className="panorama-canvas" ref={containerRef} role="region" aria-label={`Interaktives 360°-Panorama: ${site.label}`} />{site.id === "first" && showAreas && <button type="button" className="pano-area-focus" onClick={() => viewerRef.current?.lookAt(5, 140, 80, 700)}>Sperrzone ansehen</button>}{import.meta.env.DEV && ['localhost', '127.0.0.1', '[::1]'].includes(window.location.hostname) && <PanoramaPositionPicker site={site} markers={editableMarkers} point={pickedPosition} selecting={selectingPosition} onSelect={setSelectingPosition} onReset={resetPickedPosition} onOpenChange={open => { editingMarkers.current = open; }} />}</div>;
}
