...
function MeteoPage() {
  const [activeId, setActiveId] = useState(grundMeteoStation.id);
  ...
  const activeStation = windFeed.stations.find((station) => station.id === activeId) || windFeed.stations[0];
  const activeWebcam = meteoWebcams[activeWebcamIndex];
  const primaryStations = windFeed.stations.filter((station) => station.primary);
  ...
  return <div className="meteo-page">
    ...
    <section className="shell meteo-overview" aria-labelledby="wind-heading">
      ...
      <div className="wind-grid" role="tablist" aria-label="Nahe Messstation auswählen">{primaryStations.map((station) => <WindStationCard key={station.id} station={station} selected={station.id === activeStation.id} onSelect={() => showStationDetails(station.id)} />)}</div>
    ...
    <section className="shell meteo-detail" id="meteo-station-detail" ref={detailsRef} role="tabpanel" aria-label={`Details für ${activeStation.name}`}>
      <div className="station-detail-main"><p className="eyebrow">Ausgewählte Station</p><h2>{activeStation.name}</h2><p>Quelle: {activeStation.attribution} · {activeStation.altitude} m · {activeStation.id === grundMeteoStation.id ? activeStation.detail : `${activeStation.distanceKm.toFixed(1)} km ab Grindelwald`}</p><p className="station-detail-status">{activeStation.statusLabel}{activeStation.observedAt && ` · Messung ${formatWindTime(activeStation.observedAt, { day: "2-digit", month: "2-digit" })}`}{activeStation.id === grundMeteoStation.id && <> · <ExternalLink href={grundMeteoStation.mapUrl}>burnair Map öffnen ↗</ExternalLink></>}</p><div className="detail-reading"><span><strong>{readingValue(activeStation.average)}</strong><small>km/h Mittel</small></span><span><strong>{readingValue(activeStation.gust)}</strong><small>km/h Böen</small></span><span><strong>{activeStation.directionLabel}</strong><small>{activeStation.direction === null ? "–" : `${Math.round(activeStation.direction)}°`}</small></span><span><strong>{activeStation.temperature === null ? "–" : `${Math.round(activeStation.temperature)}°`}</strong><small>Temperatur</small></span></div></div>
      <div className="station-trend"><p className="eyebrow">Entwicklung</p><strong>{activeStation.trend}</strong><p>{activeStation.values.length > 1 ? "Die letzten vier verfügbaren Messungen. Messzeiten in Schweizer Ortszeit." : activeStation.values.length === 1 ? "Aktuell ist eine Messung verfügbar. Weitere Werte erscheinen, sobald die Datenquelle einen Verlauf liefert." : "Sobald Messwerte vorliegen, erscheint hier der Verlauf."}</p><div className="trend-values">{activeStation.values.slice().reverse().map((value) => <span key={value.epoch}><small>{value.time}</small><i style={{ height: `${Math.max(22, (value.gust ?? 0) * 1.5)}px` }} /><strong>{readingValue(value.average)}/{readingValue(value.gust)}</strong></span>)}</div><small>Ø / Böe in km/h</small></div>
    </section>
    <WeatherForecast />
