...
export function createMarkerSaveMiddleware(root) {
  ...
  return async (req, res, next) => {
    ...
    try {
      ...
      const { sceneId, kind, id, position } = data;
      if (!Object.hasOwn(panoramaLinks, sceneId) || !position || !Number.isFinite(position.yaw) || Math.abs(position.yaw) > 180 || !Number.isFinite(position.pitch) || Math.abs(position.pitch) > 90) throw new Error();
    ...
    const save = pending.then(async () => {
      ...
      entries[id] = { ...entries[id], yaw: position.yaw, pitch: position.pitch, provisional: false };
