...
- `src/useWindFeed.js` loads the feed on page mount and schedules the next load five minutes after each completed load while the page remains open. A local timer ages observations once per minute. The feed client keeps its five-minute cache, coalesces overlapping requests and applies the same cooldown to failed attempts. Server refresh decisions must use server-side successful-fetch state, not this browser cache timestamp.
