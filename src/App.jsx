import { relatedNews } from "./related-news.js";
...
import { ProtocolPdfViewer } from "./ProtocolPdfViewer.jsx";
...
const navItems = [routes.news, routes.meteo, routes.flightArea, routes.club, routes.photos]; const routeList = Object.values(routes);
const siteBase = import.meta.env.BASE_URL;
const protocolYearForPath = (path) => { const match = path.match(/^\/club\/protokoll-hv-(\d{4})$/); const year = Number(match?.[1]); return protocolYears.includes(year) ? year : null; };
function AppLink({ to, children, className = "", onNavigate }) { const href = appPath(to, siteBase); const go = (event) => { if (event.button !== 0 || event.metaKey || event.ctrlKey || event.shiftKey || event.altKey) return; event.preventDefault(); history.pushState({}, "", href); dispatchEvent(new PopStateEvent("popstate")); onNavigate?.(); }; return <a className={className} href={href} onClick={go}>{children}</a>; }
...
function titleForPath(path) { const protocolYear = protocolYearForPath(path); if (protocolYear) return `Protokoll HV ${protocolYear}`; if (path.startsWith("/news/")) return news.find((item) => item.path === path)?.title; if (path.startsWith("/fotos/")) return photoReports.find((item) => item.path === path)?.title; return routeList.find((item) => item.path === path)?.label; }
function Shell({ children, currentPath }) { const [menuOpen, setMenuOpen] = useState(false); useEffect(() => { setMenuOpen(false); const title = titleForPath(currentPath); document.title = title ? `${title} | Jungfrau-Tächi Grindelwald` : "Jungfrau-Tächi Grindelwald"; }, [currentPath]); return <><a className="skip-link" href="#inhalt">Zum Inhalt springen</a><header><div className="utility"><div className="shell utility-inner">{utilityLinks.map((item) => item.to ? <AppLink key={item.label} to={item.to} className={item.className}>{item.label}</AppLink> : <ExternalLink key={item.label} href={item.href}>{item.label}</ExternalLink>)}<AppLink className="utility-alert" to={routes.safety.path}>Bitte beachte die lokalen Hinweise im Fluggebiet →</AppLink></div></div><div className="shell primary-nav"><AppLink to="/" className="wordmark">{CONFIG.showAnniversary ? <img className="anniversary-logo" src={`${siteBase}assets/brand/jungfrau-taechi-50.png`} alt="Jungfrau-Tächi Grindelwald – 50 Jahre, seit 1976" width="2731" height="1347" /> : <><span>Jungfrau-Tächi</span><small>GRINDELWALD</small></>}</AppLink>{CONFIG.showAnniversary && <AppLink className="header-anniversary" to={routes.chronology.path}><small>1976–2026</small>Jubiläum entdecken →</AppLink>}<button className="menu-button" aria-expanded={menuOpen} aria-controls="site-navigation" onClick={() => setMenuOpen(!menuOpen)}>Menü</button><nav id="site-navigation" className={menuOpen ? "is-open" : ""} aria-label="Hauptnavigation">{navItems.map((item) => <AppLink key={item.path} to={item.path} className={currentPath === item.path || currentPath.startsWith(`${item.path}/`) ? "active" : ""} onNavigate={() => setMenuOpen(false)}>{item.label}</AppLink>)}<AppLink to={routes.chronology.path} onNavigate={() => setMenuOpen(false)}>Chronik</AppLink><AppLink to={routes.contact.path} onNavigate={() => setMenuOpen(false)}>Kontakt</AppLink><AppLink to={routes.membership.path} className="join-link" onNavigate={() => setMenuOpen(false)}>Mitglied werden</AppLink></nav></div></header><main id="inhalt" tabIndex="-1">{children}</main><footer><div className="shell footer-grid"><div><strong>Jungfrau-Tächi Grindelwald</strong><p>Gut Flug und immer Happy Landing!</p></div><div><AppLink to={routes.news.path}>News</AppLink><AppLink to={routes.flightArea.path}>Fluggebiet</AppLink><AppLink to={routes.meteo.path}>Meteo</AppLink><AppLink to={routes.membership.path}>Mitglied werden</AppLink></div><div><AppLink to={routes.imprint.path}>Impressum</AppLink><AppLink to={routes.contact.path}>Kontakt</AppLink></div></div></footer></>; }
...
function AnniversaryModule() { if (!CONFIG.showAnniversary) return null; return <section className="anniversary-module"><div className="shell anniversary-grid"><img src={images.challenge} alt="Karte der Jubiläums-Challenge in der Jungfrau-Region" /><div><p className="eyebrow">50 Jahre Jungfrau-Tächi</p><h2>Ein Sommer voller Flugerlebnisse</h2><p>Die Jubiläums-Challenge vom 1. Mai bis 31. August 2026 ist abgeschlossen. Entdecke die Idee hinter der Challenge und 50 Jahre Clubgeschichte.</p><div className="button-row"><AppLink className="text-link" to="/news/50challenge">Zur Challenge →</AppLink><AppLink className="text-link" to={routes.chronology.path}>Unsere Geschichte →</AppLink></div></div></div></section>; }
function Home() {
  const nextEvent = upcomingEvents(clubProgramme)[0];
  const leadNews = news[0];
  const moreNews = news.slice(1, 4);
  const latestPhotos = photoReports[0];
  return <>
    <section className="page-banner home-banner" style={{ "--page-banner-image": `url("${images.homeHero}")`, "--page-banner-position": "center 30%" }}><div className="shell home-banner-content"><p className="eyebrow">Jungfrau-Tächi Grindelwald</p><h1 tabIndex="-1"><span className="home-title-wide">Fliegen zwischen Eiger, Mönch und Jungfrau</span><span className="home-title-small">Dein Fluggebiet.<br />Grindelwald.</span></h1><p>Dein Start in die Jungfrauregion: Fluggebiet, Flugwetter und Clubleben.</p><div className="button-row"><AppLink className="button primary" to={routes.flightArea.path}>Fluggebiet entdecken</AppLink><AppLink className="button secondary" to={`${routes.flightArea.path}#flugvorbereitung`}>Zum ersten Mal hier?</AppLink></div></div></section>
    ...
    <section className="home-next-event"><div className="shell home-next-event-grid"><div><p className="eyebrow">{nextEvent ? "Nächster Clubtermin" : "Clubleben"}</p>{nextEvent && <time dateTime={nextEvent.startDate}>{nextEvent.date}</time>}</div><div><h2>{nextEvent?.title || "Gemeinsam in die nächste Saison"}</h2><p>{nextEvent?.text || "Neue Termine findest du im Jahresprogramm."}</p>{nextEvent?.path && <AppLink className="text-link" to={nextEvent.path}>Infos zum Anlass →</AppLink>}</div><AppLink className="text-link" to={`${routes.club.path}#clubleben`}>Ganzes Jahresprogramm →</AppLink></div></section>
    ...
    <section className="home-discover"><div className="shell"><header className="home-section-heading"><div><p className="eyebrow">Mehr entdecken</p><h2>Erlebnisse und Clubgeschichte</h2></div><AppLink className="text-link" to={routes.photos.path}>Alle Fotoreports →</AppLink></header><div className="home-discover-grid">{latestPhotos && <AppLink className="home-discover-card" to={latestPhotos.path}><img src={latestPhotos.image} alt={latestPhotos.alt} loading="lazy" /><span><small>Fotoreports</small><strong>{latestPhotos.title}</strong><em>Fotoreport ansehen →</em></span></AppLink>}<AppLink className="home-discover-card" to={routes.club.path}><img src={images.clubCommunity} alt="Jungfrau-Tächi beim Clubfliegen auf First" loading="lazy" /><span><small>Seit 1976</small><strong>Gemeinsam in der Luft</strong><em>Den Club kennenlernen →</em></span></AppLink></div></div></section>
    <section className="shell home-welcome"><div><p className="eyebrow">Gemeinsam in der Luft</p><h2>Dein Club. Dein Fluggebiet.</h2><p>Mit deinem Beitrag unterstützen wir den Erhalt unserer Start- und Landeplätze. Dazu kommen gemeinsame Flüge, Ausflüge und Begegnungen.</p></div><AppLink className="button primary" to={routes.membership.path}>Mitglied werden</AppLink></section>
    <AnniversaryModule />
  </>;
}
...
function WebcamCard({ camera, refreshToken, index, total, onPrevious, onNext }) {
  ...
  const imageUrl = camera.image ? `${camera.image}${refreshToken ? `${camera.image.includes("?") ? "&" : "?"}refresh=${refreshToken}` : ""}` : null;
  const focusPanorama = () => {
...
function MeteoPage() {
  ...
  const primaryStations = windFeed.stations.filter((station) => station.primary).sort((a, b) => (a.id === "windline-4104" ? -1 : b.id === "windline-4104" ? 1 : 0));
  const regionalStations = windFeed.stations.filter((station) => !station.primary);
  ...
  return <div className="meteo-page">
    ...
    <section className="shell meteo-overview" aria-labelledby="wind-heading">
      ...
      {showRegionalStations && <section className="regional-stations" id="regional-wind-stations" aria-labelledby="regional-stations-title"><header><div><p className="eyebrow">Erweiterte Region</p><h3 id="regional-stations-title">Alle weiteren Stationen</h3></div><p>Sortiert nach Entfernung zu Grindelwald. Aktuelle Werte und Verlauf direkt in jeder Karte.</p></header><div className="wind-grid is-regional" aria-label="Regionale Windstationen">{regionalStations.map((station) => <WindStationCard key={station.id} station={station} />)}</div></section>}
    </section>
    <WeatherForecast />
    <section className="meteo-secondary"><div className="shell meteo-secondary-grid">
      <div className="webcam-panel" id="webcams" tabIndex="-1"><div className="meteo-section-head"><div><p className="eyebrow">Sicht vor Ort</p><h2>Live-Webcams</h2></div><button className="webcam-refresh" type="button" onClick={() => setWebcamRefresh(Date.now())}>Bilder neu laden</button></div><label className="webcam-select">Webcam auswählen <select value={activeWebcamIndex} onChange={(event) => setActiveWebcamIndex(Number(event.target.value))}>{meteoWebcams.map((camera, index) => <option key={camera.id} value={index}>{camera.title}</option>)}</select></label><WebcamCard key={activeWebcam.id} camera={activeWebcam} refreshToken={webcamRefresh} index={activeWebcamIndex} total={meteoWebcams.length} onPrevious={() => moveWebcam(-1)} onNext={() => moveWebcam(1)} /><p className="webcam-credit">{activeWebcam.credit || "© Jungfraubahnen · Roundshot"}</p></div>
      <aside className="dabs-panel"><div className="dabs-heading"><p className="eyebrow">Luftraum · vor jedem Flug</p><h2>DABS</h2></div><div className="dabs-status"><span>!</span><div><strong>Aktuelle Luftraumhinweise prüfen</strong><p>Aktivierungen von LS-R6 und temporäre Einschränkungen im offiziellen Daily Airspace Bulletin prüfen. Hier wird kein Live-Status angezeigt.</p></div></div><div className="dabs-actions"><ExternalLink className="button dabs-button" href="https://www.skybriefing.com/de/">Offizielles DABS öffnen</ExternalLink><p className="dabs-footnote">Externer Link · Skybriefing</p></div></aside>
    </div></section>
  </div>;
}
...
function ClubDocuments() {
  ...
  const latestProtocolYear = protocolYears[0];
  const protocolPath = (year) => appPath(`/club/protokoll-hv-${year}`, siteBase);
  return <section className="club-documents" id="dokumente" aria-labelledby="club-documents-title">
    <div className="shell">
      <header><p className="eyebrow">Vereinsdokumente</p><h2 id="club-documents-title">Dokumente</h2><p>Statuten und Protokolle der Hauptversammlungen.</p></header>
      <div className="club-document-cards">
        <article><FileText size={64} weight="light" aria-hidden="true" /><div><h3>Statuten</h3><p>Die Grundlagen unseres Vereins.</p><button type="button" onClick={() => togglePanel("statuten")} aria-expanded={openPanel === "statuten"} aria-controls="club-statutes">Statuten lesen <span aria-hidden="true">→</span></button></div></article>
        <article><FileText size={64} weight="light" aria-hidden="true" /><div><h3>Protokoll HV {latestProtocolYear}</h3><p>Das Protokoll der letzten publizierten Hauptversammlung.</p><div className="club-document-actions"><a href={protocolPath(latestProtocolYear)} target="_blank" rel="noreferrer">Protokoll öffnen →</a><button type="button" onClick={() => togglePanel("protokolle")} aria-expanded={openPanel === "protokolle"} aria-controls="club-protocols">Alle Protokolle <span aria-hidden="true">{openPanel === "protokolle" ? "–" : "+"}</span></button></div></div></article>
      </div>
      {openPanel === "protokolle" && <div className="club-document-panel club-protocol-panel" id="club-protocols"><div><p className="eyebrow">Archiv 2001–2025</p><h3>Protokolle der Hauptversammlungen</h3></div><div className="club-protocol-grid">{protocolYears.map((year) => <a key={year} href={protocolPath(year)} target="_blank" rel="noreferrer"><span>HV</span><strong>{year}</strong><em>PDF öffnen →</em></a>)}</div></div>}
      {openPanel === "statuten" && <div className="club-document-panel club-statutes" id="club-statutes"><header><div><p className="eyebrow">Genehmigt am 30. Januar 2016</p><h3>Statuten Jungfrau-Tächi Grindelwald</h3></div><button type="button" onClick={() => setOpenPanel("")}>Schliessen</button></header><div className="club-statutes-grid">{statuteSections.map((section) => <section key={section.title}><h4>{section.title}</h4>{section.paragraphs?.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}{section.items && <ol>{section.items.map((item) => <li key={item}>{item}</li>)}</ol>}{section.groups?.map((group) => <div key={group.title}><h5>{group.title}</h5>{group.paragraphs?.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}{group.items && <ul>{group.items.map((item) => <li key={item}>{item}</li>)}</ul>}</div>)}</section>)}</div></div>}
    </div>
  </section>;
}
function ClubPage() {
  ...
  return <div className="club-page">
    <section className="page-banner club-intro" style={{ "--page-banner-image": `url("${images.clubHeader}")`, "--page-banner-position": "center 46%" }}><div className="shell club-intro-grid"><div><p className="eyebrow">Gemeinsam in der Luft · seit 1976</p><h1 tabIndex="-1">Jungfrau-Tächi Grindelwald</h1><p>Aus 13 Flugenthusiasten entstand ein Club mit 361 Mitgliedern und einem der bekanntesten gebührenfreien Fluggebiete der Schweiz.</p><AppLink className="button primary" to={routes.membership.path}>Mitglied werden</AppLink></div><div className="club-numbers"><span><strong>1976</strong><small>gegründet</small></span><span><strong>361</strong><small>Mitglieder</small></span><span><strong>50</strong><small>Jahre Clubgeschichte</small></span></div></div></section>
    <section className="club-agenda" id="clubleben" tabIndex="-1"><div className="shell"><header className="club-agenda-heading"><div><p className="eyebrow">Jahresprogramm 2026/27</p><h2>Als Nächstes im Club</h2></div><p>Fliegen, feiern und gemeinsam unterwegs sein: die kommenden Termine auf einen Blick.</p></header><div className="club-agenda-grid">{upcomingEvents(clubProgramme).map((event) => <article key={`${event.date}-${event.title}`}><time dateTime={event.startDate}>{event.date}</time><h3>{event.title}</h3><p>{event.text}</p></article>)}</div></div></section>
    ...
    <section className="shell club-portrait" id="portrait"><div className="club-portrait-media"><img src={images.clubCommunity} alt="Jungfrau-Tächi beim Clubfliegen auf First" loading="lazy" /><span>Clubfliegen auf First</span></div><div className="club-portrait-copy"><p className="eyebrow">Portrait · von Urs Dubach</p><h2>Von 13 Pionieren zu 361 Tächi</h2>{clubPortrait.slice(0, 2).map((paragraph) => <p key={paragraph}>{paragraph}</p>)}<details className="club-portrait-more"><summary>Vollständiges Portrait lesen</summary><div>{clubPortrait.slice(2).map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div></details></div></section>
...
function MembershipPage() { const embedUrl = `${membershipFormUrl}?embedded=true`; return <><SectionIntro eyebrow="Club" title="Werde Teil der Jungfrau-Tächi" body="Ob aktiv in der Luft oder verbunden mit dem Club: Wir freuen uns über neue Mitglieder." image={images.hero} position="center 42%" /><section className="shell membership-layout"><div className="membership-steps"><p className="eyebrow">So funktioniert es</p><h2>In wenigen Schritten dabei</h2><ol><li><strong>Anmeldeformular ausfüllen</strong><span>Persönliche Angaben und gewünschte Mitgliedschaft eintragen.</span></li><li><strong>Formular absenden</strong><span>Der Vorstand prüft deine Anmeldung und meldet sich bei dir.</span></li><li><strong>Willkommen im Club</strong><span>Informiere dich auf unserer Webseite über die nächsten Anlässe.</span></li></ol></div><div className="membership-form"><header><p className="eyebrow">Anmeldung</p><h2>Mitgliedschaft beantragen</h2><p>Fülle das Google-Formular direkt hier aus. Pflichtfelder sind mit einem Stern gekennzeichnet.</p></header><iframe src={embedUrl} title="Anmeldeformular Clubmitgliedschaft Jungfrau-Tächi Grindelwald" loading="lazy">Anmeldeformular wird geladen.</iframe><p className="membership-form-fallback">Formular nicht sichtbar? <ExternalLink href={membershipFormUrl}>In Google Forms öffnen</ExternalLink></p></div></section></>; }
function FlightAreaPage() { return <div className="flight-area-page"><section className="flight-explorer-intro"><div className="shell"><div><p className="eyebrow">Fluggebiet Jungfrauregion · 360°</p><h1 tabIndex="-1">Das Fluggebiet aus der Luft und am Boden</h1><p>Wechsle direkt zwischen vier Übersichten, drei Startplätzen und zwei Landeplätzen im Grindelwaldtal. Die Panoramen erleichtern die Orientierung vor dem Flug.</p></div><div className="flight-intro-actions"><AppLink to={routes.meteo.path}>Meteo prüfen</AppLink><AppLink className="safety-link" to={routes.safety.path}>Sicherheit</AppLink><ExternalLink href="https://www.skybriefing.com/de/">DABS öffnen</ExternalLink></div></div></section><FlightExplorer /><section className="shell flight-overview" id="flugvorbereitung" tabIndex="-1"><header className="home-section-heading"><div><p className="eyebrow">Gut vorbereitet starten</p><h2>Zum ersten Mal in Grindelwald?</h2><p>Orientiere dich an Start- und Landeplätzen, prüfe die lokalen Regeln und hole vor dem Flug aktuelle Wetter- und Luftrauminformationen ein.</p></div></header><div className="flight-fact-grid">{flightFacts.map((fact, index) => <AppLink key={fact.title} to={fact.path}><span>0{index + 1}</span><h3>{fact.title}</h3><p>{fact.body}</p><strong>Öffnen →</strong></AppLink>)}</div></section></div>; }
...
function FlightExplorer({ initialGroup = "overview", initialSceneId = "" }) {
  ...
  return <section className="shell flight-explorer" aria-labelledby="flight-explorer-title">
    <header className="flight-explorer-heading"><div><p className="eyebrow">Interaktiver Gebiets-Explorer</p><h2 id="flight-explorer-title">9 Standorte in einer Ansicht</h2></div><p>Zur Orientierung vor dem Flug. Prüfe die aktuelle Situation und lokale Hinweise immer vor Ort.</p></header>
...
function FirstTakeoffNotice() { return <aside className="first-takeoff-notice"><div><p className="eyebrow">Startplatz First</p><h3>Sperrzone östlich vom Weg</h3><p>Östlich des Wegs, oberhalb des Holzturms in Richtung Firstbahn-Bergstation, sind Paragliding-Starts verboten. Beachte die Abgrenzung in der Draufsicht und die Hinweise vor Ort.</p><ExternalLink className="text-link" href={siteBase + "assets/flight-area/startfirst.jpg"}>Grafik vergrössern ↗</ExternalLink></div><a href={siteBase + "assets/flight-area/startfirst.jpg"} target="_blank" rel="noreferrer" aria-label="Sperrzonen-Grafik First vergrössern (neuer Tab)"><img src={siteBase + "assets/flight-area/startfirst.jpg"} alt="Startplatz First: rot markierte Sperrzone für Paragliding-Starts östlich des Wegs" width="1339" height="1800" loading="lazy" /></a></aside>; }
function StartSitesPage() { return <><SectionIntro eyebrow="Fluggebiet · 360°" title="Startplätze" body="First, Waldspitz und Männlichen im gemeinsamen Gebiets-Explorer." /><FlightExplorer initialGroup="start" /><FlightPageNav /></>; }
function LandingSitesPage() { return <><SectionIntro eyebrow="Fluggebiet · 360°" title="Landeplätze" body="Grund und Bodmi im gemeinsamen Gebiets-Explorer." /><FlightExplorer initialGroup="landing" /><FlightPageNav /></>; }
...
function SafetyPage() { return <div className="safety-page"><SectionIntro eyebrow="Fluggebiet" title="Sicherheit und lokale Regeln" body="Diese Übersicht erleichtert die Vorbereitung. Verbindlich bleiben die aktuellen offiziellen Luftfahrtinformationen, DABS und Hinweise vor Ort." /><section className="shell safety-priority"><div><span>!</span><div><strong>DABS vor jedem Flug prüfen</strong><p>Aktivierungen und temporäre Lufträume können sich kurzfristig ändern.</p></div></div><ExternalLink className="button primary" href="https://www.skybriefing.com/de/">Offizielles DABS öffnen</ExternalLink></section><section className="shell shv-reference"><div><p className="eyebrow">Offizielle Fluggebietsinformation · 06/2024</p><h2>SHV Infotafel Grindelwald</h2><p>Übersichtskarte mit Startplatz First, Landeplätzen Grund und Bodmi, Sonderregelungen, Wildschutzgebieten, Kabeln und angrenzenden Lufträumen.</p></div><div className="button-row"><ExternalLink className="button secondary" href={shvGrindelwaldDocument}>PDF öffnen</ExternalLink><ExternalLink className="button secondary" href={shvAirspaceUrl}>SHV Airspace Map</ExternalLink></div></section><section className="shell"><FirstTakeoffNotice /></section><section className="shell safety-layout"><aside><p className="eyebrow">Bereiche</p>{safetyAreas.map((area) => <a key={area.id} href={`#${area.id}`}>{area.title}</a>)}</aside><div className="safety-content">{safetyAreas.map((area) => <article id={area.id} key={area.id}><header><p className="eyebrow">{area.detail}</p><h2>{area.title}</h2><p>{area.body}</p>{area.document && <div className="button-row safety-document"><ExternalLink className="button secondary" href={area.document.path}>{area.document.title} · {area.document.edition}</ExternalLink></div>}</header>{area.document?.preview && <div className="safety-images"><a href={area.document.path} target="_blank" rel="noreferrer" aria-label={`${area.document.title} als PDF öffnen (neuer Tab)`}><img src={area.document.preview} alt={`${area.title} – Vorschau der PDF-Karte`} loading="lazy" /><span>PDF öffnen</span></a></div>}{area.graphic && <figure className="safety-graphic"><a href={area.graphic} target="_blank" rel="noreferrer" aria-label={`${area.title} – Grafik vergrössern (neuer Tab)`}><img src={area.graphic} alt="Karte des Jagdbanngebiets nördlich von First mit markiertem Schutzgebiet" loading="lazy" /></a><figcaption>Von Pascal Imhof bereitgestellter Kartenausschnitt. Die aktuellen Grenzen im <ExternalLink href="https://map.geo.admin.ch/">Geoportal des Bundes</ExternalLink> mit dem Filter «AuLaV» prüfen.</figcaption></figure>}{area.images.length > 0 && <div className={`safety-images${area.images.length > 1 ? " is-multiple" : ""}`}>{area.images.map((image, index) => <a key={image} href={area.document?.path || image} target="_blank" rel="noreferrer"><img src={image} alt={`${area.title} – Regelkarte ${index + 1}`} loading="lazy" /><span>{area.document ? "PDF öffnen" : "Vergrössern"}</span></a>)}</div>}</article>)}</div></section><FlightPageNav /></div>; }
...
function ImprintPage() { return <><SectionIntro eyebrow="Jungfrau-Tächi" title="Impressum" /><Imprint /></>; }
function NotFound() { return <SectionIntro eyebrow="Nicht gefunden" title="Diese Seite ist nicht verfügbar." />; }
function RouteView({ path }) { if (path.startsWith("/news/")) return <NewsArticlePage article={news.find((item) => item.path === path)} />; if (path.startsWith("/fotos/")) return <PhotoReportPage report={photoReports.find((item) => item.path === path)} />; const views = { "/": Home, "/meteo": MeteoPage, "/news": NewsPage, "/club": ClubPage, "/chronik": ChronologyPage, "/mitglied": MembershipPage, "/fluggebiet": FlightAreaPage, "/fluggebiet/startplaetze": StartSitesPage, "/fluggebiet/landeplaetze": LandingSitesPage, "/fluggebiet/sicherheit": SafetyPage, "/fluggebiet/grund": GrundPage, "/fotos": PhotosPage, "/kontakt": ContactPage, "/impressum": ImprintPage }; const View = views[path] || NotFound; return <View />; }
export function resolveRoute(pathname) { const routePath = routeFromPathname(pathname, siteBase); if (routeList.some((route) => route.path === routePath)) return routePath; if (news.some((item) => item.path === routePath) || photoReports.some((item) => item.path === routePath) || protocolYearForPath(routePath)) return routePath; return null; }
export function App() {
  const currentLocation = () => ({ path: routeFromPathname(window.location.pathname, siteBase), hash: window.location.hash });
  const [location, setLocation] = useState(currentLocation);
  const initialLocation = useRef(location);
  useEffect(() => { const update = () => setLocation(currentLocation()); addEventListener("popstate", update); addEventListener("hashchange", update); return () => { removeEventListener("popstate", update); removeEventListener("hashchange", update); }; }, []);
  useEffect(() => {
    if (location === initialLocation.current && !location.hash) return;
    const frame = requestAnimationFrame(() => {
      ...
      else { window.scrollTo(0, 0); document.querySelector("#inhalt h1")?.focus({ preventScroll: true }); }
    });
    return () => cancelAnimationFrame(frame);
  }, [location]);
  const protocolYear = protocolYearForPath(location.path);
  if (protocolYear) return <ProtocolPdfViewer year={protocolYear} />;
  return <Shell currentPath={location.path}><RouteView path={resolveRoute(location.path) || location.path} /></Shell>;
}
