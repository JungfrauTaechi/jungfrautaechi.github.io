...
export function PanoramaPositionPicker({ site, markers, point, selecting, onSelect, onReset, onOpenChange }) {
  ...
  const save = async () => {
    ...
    setStatus('');
    try {
      const response = await fetch('/__local/marker-position', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sceneId: site.id, kind: selected.section || 'webcam', id: selected.id, position: point }) });
  ...
  return <div className="pano-marker-editor" onPointerDown={(event) => event.stopPropagation()}>
    ...
    {open && <div>
      ...
      <button type="button" disabled={saving} aria-pressed={selecting} onClick={() => { setStatus(''); onSelect(!selecting); }}>{selecting ? "Abbrechen" : "Position im Bild wählen"}</button>
      ...
      {point && <><button type="button" disabled={saving || selecting} onClick={save}>{saving ? 'Speichert …' : 'Position speichern'}</button><details><summary>Koordinaten kopieren</summary><p>Unter <code>{site.id}</code> in <code>{selected.file}</code>:</p><textarea aria-label="Markerkoordinaten zum Kopieren" readOnly value={snippet} rows={8} onFocus={(event) => event.target.select()} /></details></>}
